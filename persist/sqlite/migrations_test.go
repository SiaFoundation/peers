...
CREATE INDEX syncer_peers_is_full_node_idx ON syncer_peers (peer_address, failure_rate, is_full_node, last_successful_scan);
...
func TestMigrationConsistency(t *testing.T) {
	...
	fp := filepath.Join(t.TempDir(), "nomad.sqlite3")
